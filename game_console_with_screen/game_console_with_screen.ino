...
void setup() {
  Serial.begin(9600);
  while (!Serial)
    ;  //dont started till serial port isnt open
  ...
  // //Look for only our chosen UID's

  display.setRotation(1);
  display.clearDisplay();
  drawText(F("PLEASE    SCAN A    VALID CARD"), 0, 42);  //ask Prof O'Loghlin about string literals
  // free(textstring);
  display.display();
  while (true) {
    // char* textstring = (char*)malloc(sizeof("PLEASE SCAN VALID CARD :D"));
    // textstring = "PLEASE SCAN VALID CARD :D";
