...
void holdBlocks(){
  // Don't execute if the game is over, paused, or the player already held this turn
  ...
  if (holdType == -1) {
    ...
    // Reset position and rotation, similar to your generate() function
    pieceX = 3; 
    pieceY = 0;
    rotation = 0;
    copyPiece(piece, currentType, rotation);
  ...
  canHold = false; // Prevent holding again until a new piece locks in
  refreshGrid();   // Update the display
  delay(200);      // Simple debounce for the button
}

// void garbageBlocks(){} - optional
