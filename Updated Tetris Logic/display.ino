...
void drawHoldPiece() {
  if (holdType == -1) return; // Do nothing if the hold slot is empty

  short hPiece[2][4];
  copyPiece(hPiece, holdType, 0); // Get the held piece geometry at rotation 0
  
  for (short i = 0; i < 4; i++) {
    // Drawn at X=30 so it sits between the score and the Next piece
    display.fillRect(30 + 3 * hPiece[0][i], 4 + 3 * hPiece[1][i], 2, 2, WHITE);
  }
}
